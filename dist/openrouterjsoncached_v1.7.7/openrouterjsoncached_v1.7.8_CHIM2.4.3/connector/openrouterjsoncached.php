...
class openrouterjsoncached
{
    ...
    const VERSION = 'OpenRouter Cache Connector v1.7.8 for CHIM 2.4.3+ | 2026/04/23';
    ...
    private function _openPart4($customParms, $herikaName, $MAX_TOKENS, $toggleThinking, $thinkingTokens,
        ...
        if (!empty($GLOBALS["CONNECTOR"][$this->name]["PROVIDER"])) {
            $providers = array_map('trim', explode(",", $GLOBALS["CONNECTOR"][$this->name]["PROVIDER"]));
            $data["provider"] = array(
                "order" => $providers,
        ...
        // Add optional provider routing settings
        $connCfg = $GLOBALS["CONNECTOR"][$this->name];
        if (!empty($connCfg["providers_sort"]) && $connCfg["providers_sort"] !== "default") {
            $data["provider"]["data_collection"] = "deny";
            $data["provider"]["sort"] = $connCfg["providers_sort"];
        }
        if (!empty($connCfg["providers_to_ignore"])) {
            $data["provider"]["ignore"] = array_map('trim', explode(",", $connCfg["providers_to_ignore"]));
        }
        if (!empty($connCfg["provider_quantizations"])) {
            $data["provider"]["quantizations"] = array_map('trim', explode(",", $connCfg["provider_quantizations"]));
        }
        if (!empty($connCfg["provider_max_price_input"]) && floatval($connCfg["provider_max_price_input"]) > 0) {
            if (!isset($data["provider"]["max_price"])) $data["provider"]["max_price"] = [];
            $data["provider"]["max_price"]["prompt"] = floatval($connCfg["provider_max_price_input"]);
        }
        if (!empty($connCfg["provider_max_price_output"]) && floatval($connCfg["provider_max_price_output"]) > 0) {
            if (!isset($data["provider"]["max_price"])) $data["provider"]["max_price"] = [];
            $data["provider"]["max_price"]["completion"] = floatval($connCfg["provider_max_price_output"]);
        }
